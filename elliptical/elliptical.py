...
def inscribe_ellipse_in_3d(col_vertices):
    ...
    item = [slice(None)] * col_vertices.ndim
    ...
    z_shape = list(ellipse_vertices.shape)
    z_shape[component_axis] = 1
    ellipse_vertices = np.concatenate((ellipse_vertices, np.zeros(tuple(z_shape))),
                                      axis=component_axis)
...
def inscribe_max_area_ellipse_in_parallelogram(vertices):
    ...
    """
    component_axis = -2
    h, k, a, b, c, d = parallelogram.get_equation_of_max_area_ellipse_in_parallelogram(vertices)
